...
public class CommonsTest extends HybrisJUnit4TransactionalTest
{
	...
	public void testGetandSetAllDocuments1() throws JaloBusinessException
	{
		...
		ret2.setCode("myDocument2");


		final Collection res = CommonsManager.getInstance().getAllDocuments(testorder);

		assertEquals(res.size(), 2);
		for (final Iterator iter = res.iterator(); iter.hasNext();)
		{
			final Document element = (Document) iter.next();
			assertEquals("myDocument2", element.getCode());
		}
		...
		final Collection res2 = CommonsManager.getInstance().getAllDocuments(testorder);

		assertEquals(res2.size(), 1);
		for (final Iterator iter = res2.iterator(); iter.hasNext();)
		{
			final Document element = (Document) iter.next();
			assertEquals("dddd", element.getCode());
		}
