...
public class CatalogVersionServiceTest extends ServicelayerTransactionalTest
{
	...
	@Test
	public void uniqueKeysShouldBeReportedAsDuplicates()
	{
		// given
		final CatalogVersionModel test1Winter = catalogVersionService.getCatalogVersion(TEST_CATALOG_1, WINTER_VERSION);
		final ProductModel prod1 = modelService.create(ProductModel.class);
		prod1.setCode("P-001");
		final ProductModel prod2 = modelService.create(ProductModel.class);
		prod2.setCode("P-002");
		modelService.saveAll(prod1, prod2);

		final JdbcTemplate template = new JdbcTemplate(Registry.getCurrentTenant().getDataSource());
		template.execute("update junit_products set p_code = 'P-001' where p_code = 'P-002'");
